...
local GetPrideAndBerserkedScaledDamage = function(hUnit, nDamage)
    local nDamage, nCriticalDamage = ( nDamage or 0 ), 0
    if IsNotNull(hUnit) then
        --========================================--
        local nAlliesDead   = 0
        ...
        --========================================--
        local nMaxUnitsOnMap = 13
        local nPrideScaleValue = GetAttributeValue(hUnit, "leonidas_pride_attribute", "scale_per_hero_max", -1, 0, false)
              nPrideScaleValue = 1 + ( GetLerped(0, nPrideScaleValue, ( nAlliesDead + nEnemiesAlive ) / nMaxUnitsOnMap) * 0.01 )
...
function leonidas_kick:OnSpellStart()
    ...
    local nBaseDamage, nBaseCriticalDamage     = GetPrideAndBerserkedScaledDamage(hCaster, self:GetSpecialValueFor("base_damage"))
    local nBounceDamage, nBounceCriticalDamage = GetPrideAndBerserkedScaledDamage(hCaster, self:GetSpecialValueFor("bounce_damage"))
...
function leonidas_catch:GetAOERadius()
    ...
    if type(hCaster.GetIntellect) == "function" then
        nScale = nScale + ( GetAttributeValue(hCaster, "leonidas_math_attribute", "catch_radius_pct_scale_per_int", -1, 0, false) * hCaster:GetIntellect() * 0.01 )
    end
    ...
    for _, hEntity in pairs(hEntities) do
        if IsNotNull(hEntity) then
            ...
            if nBaseCriticalDamage > 0 then
                ...
                hEntity:AddNewModifier(hCaster, self, "modifier_leonidas_berserk_damage_overtime", {duration = 3, nDamageType = DAMAGE_TYPE_PHYSICAL, nDamage = nBaseCriticalDamage})
            end
            --=================================--
            if not bCasterBerserked then
                break
        ...
        for _, hEntity in pairs(hEntities) do
            if IsNotNull(hEntity) then
                ...
                if nCriticalDamage > 0 then
                    ...
                    hEntity:AddNewModifier(hCaster, self, "modifier_leonidas_berserk_damage_overtime", {duration = 3, nDamageType = DAMAGE_TYPE_PHYSICAL, nDamage = nCriticalDamage})
                end
            end
        end
        --===============================
...
function modifier_leonidas_pride_counter:GetModifierIncomingDamage_Percentage(keys)
    if IsServer() then
        ...
        if(bCasterBerserked) then
            ...
            self.nBonusDamage = GetClamped(self.nBonusDamage, 0, self.hParent:GetMaxHealth()*0.5)
        end
...
function modifier_leonidas_berserk:GetModifierProcAttack_Feedback(keys) --NOTE: Как бы я не хотел засунуть сюда 3% от макс хп врага физикой.... я не заснул т.к долго думал над балансом... и т.д
    if IsServer()
        ...
        EmitSoundOn("Leonidas.Berserk.Crit.Universal", self.hParent)
    end
end
...
function modifier_leonidas_berserk:OnIntervalThink()
    if IsServer()
        ...
        for _, hEntity in pairs(hEntities) do
            if IsNotNull(hEntity) then
                local nCalculatedDamage = self.nPurgeDamage + ( ( self.hParent:GetHealthDeficit() * self.nMissingHPDamage ) )
                local nCalculatedDamage, nCriticalDamage = GetPrideAndBerserkedScaledDamage(self.hParent, nCalculatedDamage)
                --=================================--
                ...
                if nCriticalDamage > 0 then
                    ...
                    -- SendOverheadEventMessage(nil, OVERHEAD_ALERT_DEADLY_BLOW, hEntity, nCriticalDamage, nil)
                    hEntity:AddNewModifier(self.hCaster, self.hAbility, "modifier_leonidas_berserk_damage_overtime", {duration = 3, nDamageType = DAMAGE_TYPE_PHYSICAL, nDamage = nCriticalDamage})
                end
            end
        end
        --=================================--
...
function leonidas_bc:OnOwnerDied1111()
    local hCaster = self:GetCaster()
...
function modifier_leonidas_bc_heal:GetTexture()
    return "anime_hero_leonidas/leonidas_bc_attribute"
end
...
function leonidas_enomotia:ReleaseEnomotia(hCaster, nPFX_AnimReleaseTime, nPushRadius, nRadius, nPushDuration, nPushSlowDuration, nPushSlowPct, nShieldDuration, nDamageType, nShieldDamage, nShieldCount, nBaseBlockPerShield, nBonusBlockPerPushed)
    ...
    local nBaseDamage, nBaseCriticalDamage = GetPrideAndBerserkedScaledDamage(hCaster, nShieldCount * nShieldDamage)
    --print(nBaseDamage, nBaseCriticalDamage)
    ...
    --=================================--
    local nPreviousStacks = 0--hCaster:GetModifierStackCount("modifier_leonidas_enomotia_shield", hCaster)
    --=================================--
...
function leonidas_enomotia_combo:OnSpellStart()
    ...
    if not bCasterBerserked then
        ...
        --=================================--
        local nShowShields = nShieldCount - 12 - 112
        local _nShieldsPFX =    ParticleManager:CreateParticle("particles/heroes/anime_hero_leonidas/leonidas_thermopylae_enomotia_shields_berserked.vpcf", PATTACH_WORLDORIGIN, nil)
                                ParticleManager:SetParticleShouldCheckFoW(_nShieldsPFX, false)
                                ParticleManager:SetParticleControlOrientationFLU(_nShieldsPFX, 0, vForward, -vRight, vUp)
                                ParticleManager:SetParticleControlOrientationFLU(_nShieldsPFX, 1, vForward, -vRight, vUp)
                                ParticleManager:SetParticleControl(_nShieldsPFX, 0, hCaster:GetAttachmentOrigin(hCaster:ScriptLookupAttachment("ATTACH_HITLOC")))
