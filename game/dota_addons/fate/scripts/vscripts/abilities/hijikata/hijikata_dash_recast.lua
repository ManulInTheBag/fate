    	...
    	if not self.target:IsMagicImmune() then
            Timers:CreateTimer(0.1, function()
                if self.parent.IsShinsengumiAcquired then
                    DoDamage(self.parent, self.target, self.parent:GetAverageTrueAttackDamage(hCaster) * self.ability:GetSpecialValueFor("sa_atk_dmg_mod"), DAMAGE_TYPE_MAGICAL, 0, self.ability, false)
                end
