...
function OnGOBHit(keys)
	...
	if caster.IsSumerAcquired then
		damage1 = caster:GetAttackDamage() * 0.35
	end
	...
	DoDamage(keys.caster, keys.target, damage1, DAMAGE_TYPE_PHYSICAL, 0, keys.ability, false)
...
function OnMaxEnumaStart(keys)
	...
	local masterCombo = caster.MasterUnit2:FindAbilityByName("gilgamesh_max_enuma_elish")--caster.MasterUnit2:FindAbilityByName("gilgamesh_combo_proxy")
