...
function atalanta_phoebus_catastrophe_snipe:OnSpellStart()
    ...
    self:ShootAirArrows()
    caster.snipeParticle = ParticleManager:CreateParticle("particles/custom/atalanta/atalanta_crosshair.vpcf", PATTACH_OVERHEAD_FOLLOW, target)
    ParticleManager:SetParticleShouldCheckFoW( caster.snipeParticle, false)
    ParticleManager:SetParticleControl( caster.snipeParticle, 0, target:GetAbsOrigin() + Vector(0,0,250)) 
    ParticleManager:SetParticleControl( caster.snipeParticle, 1, target:GetAbsOrigin() + Vector(0,0,250)) 
    ...
    Timers:CreateTimer(2, function()
        ...
        for i=1,arrows do
            Timers:CreateTimer(0.1 + 0.15 * i, function()
