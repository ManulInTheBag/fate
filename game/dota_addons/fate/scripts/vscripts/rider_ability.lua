...
function OnBelleStart(keys)
	...
	if caster.IsRidingAcquired then 
		keys.Damage = keys.Damage + 250 + (caster:GetAgility() * 3)
	end 
	...
	Timers:CreateTimer(0.5, function()
        ...
        if playerHero.gachi == true then
        	...
        	CustomGameEventManager:Send_ServerToPlayer(player, "emit_horn_sound", {sound="RRD"})
...
function RiderCheckCombo(caster, ability)
	if caster:GetStrength() >= 29.1 and caster:GetAgility() >= 29.1 and caster:GetIntellect() >= 29.1 then
		...
		--if ability == caster:FindAbilityByName("imba_pudge_meat_hook") then
			caster.nailUsed = true
			caster.nailTime = GameRules:GetGameTime()
			Timers:CreateTimer({
				endTime = 7,
				callback = function()
				caster.nailUsed = false
			end
			})
