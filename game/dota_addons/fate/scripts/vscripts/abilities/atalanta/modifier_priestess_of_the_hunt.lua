...
function modifier_priestess_of_the_hunt:CheckState()
    if IsServer() then
...
function modifier_priestess_of_the_hunt:OnAttack(args)
    if args.attacker ~= self:GetParent() or self:GetParent():HasModifier("modifier_tauropolos") then return end

    local modifier = self:GetParent():FindModifierByName("modifier_priestess_of_the_hunt")
    modifier:SetStackCount(math.max(modifier:GetStackCount() - 1, 0))
end

function modifier_priestess_of_the_hunt:OnAttackLanded(args)
    if args.attacker ~= self:GetParent() then return end
    args.attacker:AddHuntStack(args.target, 1)
end
...
function modifier_priestess_of_the_hunt:OnIntervalThink()
    if IsServer() then
        ...
        nextArrow = nextArrow + 1 * THINK_INTERVAL
