-- master_shard_of_replenishment — портировано из datadriven панелью (dd2lua).
-- Источник: scripts/npc/npc_abilities_custom.txt
-- Проверить: TODO-пометки ниже, прекеш и места применения модификаторов.

master_shard_of_replenishment = class({})

-- Логика перенесена из scripts/vscripts/master_ability.lua (DD-обвязка удалена).
-- Функции локальные: одноимённые глобали в разных файлах —
-- отдельный класс ловушек, повторять его незачем.
local OnReplenishmentAcquired

OnReplenishmentAcquired = function(keys)
...
function master_shard_of_replenishment:OnSpellStart()
	local caster = self:GetCaster()
	-- DD RunScript: master_ability / OnReplenishmentAcquired
	OnReplenishmentAcquired({ caster = caster, ability = self, target = caster })
end
