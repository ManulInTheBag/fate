-- Mod-wide generic CC/state modifiers (lua port of the old datadriven
-- container that lived in presence_detection_passive). Everything applied
-- through giveUnitDataDrivenModifier lives here; all of these are ubiquitous
-- (stuns, roots, round pause, jumps...), so they are linked centrally on load.
-- Hero-specific modifiers belong next to their hero, not here.

local THIS = "modifiers/cc_modifiers"
...
-- ward: 3000 phys block, magic immune, no collision, chip-damage on hit
-- (applied by the scout/sentry familiar items on their ward unit)
