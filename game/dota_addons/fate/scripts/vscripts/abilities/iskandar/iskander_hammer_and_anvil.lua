-- iskander_hammer_and_anvil — портировано из datadriven панелью (dd2lua).
-- Источник: scripts/npc/abilities/iskandar/iskandar_abilities.kv
-- Проверить: TODO-пометки ниже, прекеш и места применения модификаторов.

iskander_hammer_and_anvil = class({})

-- Логика перенесена из scripts/vscripts/iskander_ability.lua, scripts/vscripts/lancer_ability.lua (DD-обвязка удалена).
-- Функции локальные: одноимённые глобали в разных файлах —
-- отдельный класс ловушек, повторять его незачем.
local GBAttachEffect, OnHammerStart, CleanUpHammer
...
function iskander_hammer_and_anvil:GetAOERadius()
...
function iskander_hammer_and_anvil:OnAbilityPhaseStart()
	...
	GBAttachEffect({ caster = caster, ability = self, target = caster })
	return true
end

function iskander_hammer_and_anvil:OnSpellStart()
	local caster = self:GetCaster()
	-- DD RunScript: iskander_ability / OnHammerStart
	OnHammerStart({
		caster = caster,
		ability = self,
		target = caster,
