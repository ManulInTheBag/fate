...
function StartCharismaTimer(keys)
	...
	Timers:CreateTimer('charisma_passive_timer', {
		endTime = 0,
		callback = function()
		local targets = FindUnitsInRadius(caster:GetTeam(), caster:GetAbsOrigin(), nil, keys.Radius, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false)
...
function OnChariotRide(keys)
	...
	if caster.IsVEAcquired then
		if caster:FindAbilityByName("iskander_via_expugnatio"):IsHidden() then
			caster:SwapAbilities(caster:GetAbilityByIndex(2):GetName(), "iskander_via_expugnatio", false, true) 
			caster:FindAbilityByName("iskander_via_expugnatio"):EndCooldown()
		end
	else
		caster:SwapAbilities(caster:GetAbilityByIndex(2):GetName(), "fate_empty3", false, true)
	end
...
function OnChariotEnd(keys)
	...
	if caster:HasModifier("modifier_army_of_the_king_death_checker") then
		caster:SwapAbilities("fate_empty3", caster:GetAbilityByIndex(2):GetName(), true, false) 
	else
...
function OnAOTKLevelUp(keys)
	--print(aotkAbilityHandle:GetName())
