...
LinkLuaModifier("modifier_aoko_jumpback_ms", "abilities/aoko/aoko_jumpback", LUA_MODIFIER_MOTION_NONE)
...
function aoko_jumpback:GetManaCost()
	local caster = self:GetCaster()
...
function aoko_jumpback:OnSpellStart()
	...
	local beam_forward = self:GetSpecialValueFor("beam_forward")
	...
	Timers:CreateTimer(0.0, function()
		if counter >= beam_count then return end
		self:FireGroundBeam(GetGroundPosition(ori + forward*(counter*beam_forward), caster), forward, right)
		counter = counter + 1
		return 0.033
	end)
end
...
function aoko_jumpback:FireGroundBeam(position, forward, right)
	...
	for _, enemy in pairs(enemies) do
	    if enemy and not enemy:IsNull() and IsValidEntity(enemy) and not self.enemieshit[enemy:entindex()] then
	...
	for _, check in pairs(spherecheck) do
		if check and not check:IsNull() and check:HasModifier("modifier_aoko_sphere_dummy") and not self.enemieshit[check:entindex()] then
			self.enemieshit[check:entindex()] = true
			check:FindModifierByName("modifier_aoko_sphere_dummy"):JumpbackExplode()
		end
	end
...
modifier_aoko_jumpback_ms = class({})

function modifier_aoko_jumpback_ms:IsHidden() return false end
function modifier_aoko_jumpback_ms:IsDebuff() return false end
function modifier_aoko_jumpback_ms:RemoveOnDeath() return true end
function modifier_aoko_jumpback_ms:DeclareFunctions()
	return { 
        MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE
           }
end

function modifier_aoko_jumpback_ms:GetModifierMoveSpeedBonus_Percentage(keys)
