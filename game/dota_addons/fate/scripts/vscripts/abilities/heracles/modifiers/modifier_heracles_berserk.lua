...
modifier_heracles_berserk_matthias = class({})

if IsServer() then
	function modifier_heracles_berserk_matthias:OnCreated(args)
		self.LockedHealth = args.LockedHealth
		self.BonusAttSpd = (1.2 - self:GetParent():GetHealth()/self:GetParent():GetMaxHealth())*self:GetAbility():GetSpecialValueFor("bns_att_spd")
		self.BonusDamage = (1.2 - self:GetParent():GetHealth()/self:GetParent():GetMaxHealth())*100
		self.BonusMs = (1.2 - self:GetParent():GetHealth()/self:GetParent():GetMaxHealth())*100
	...
	function modifier_heracles_berserk_matthias:OnRefresh(args)
		self:OnCreated(args)
	end

	function modifier_heracles_berserk_matthias:OnDestroy()
		--self:GetParent():SetRenderColor(255, 255, 255)
	end

	function modifier_heracles_berserk_matthias:OnIntervalThink()
		local parent = self:GetParent()
	...
	function modifier_heracles_berserk_matthias:OnTakeDamage(args)
		local caster = self:GetParent()
    	if IsServer() then
        	if args.unit ~= self:GetParent() then return end
...
function modifier_heracles_berserk_matthias:DeclareFunctions()
	return { --MODIFIER_EVENT_ON_TAKEDAMAGE,
			 MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,
			 --MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE,
			 MODIFIER_PROPERTY_BASEDAMAGEOUTGOING_PERCENTAGE,
...
function modifier_heracles_berserk_matthias:GetModifierIncomingDamage_Percentage() 
	return -20
end


function modifier_heracles_berserk_matthias:GetModifierBaseDamageOutgoing_Percentage()
	local bonus_damage = 0
...
function modifier_heracles_berserk_matthias:GetModifierAttackSpeedBonus_Constant()
	local bonus_attspd = 0
...
function modifier_heracles_berserk_matthias:GetModifierMoveSpeedBonus_Percentage()
	local bonus_ms = 0
...
function modifier_heracles_berserk_matthias:RemoveOnDeath()
	return true 
end

function modifier_heracles_berserk_matthias:GetTexture()
	return "custom/berserker_5th_berserk"
end

function modifier_heracles_berserk_matthias:GetEffectName()
	return "particles/custom/berserker/berserk/buff.vpcf"
end

function modifier_heracles_berserk_matthias:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_heracles_berserk_matthias:GetStatusEffectName()
	return "particles/custom/berserker/berserk/berserk_status_fx_matthias.vpcf"
end

function modifier_heracles_berserk_matthias:StatusEffectPriority()
    return 1000
end
