...
//--------------------------------------------------------------------------------------------------
// Сколько РЕАЛЬНЫХ игроков ещё не сидят в команде.
// ⚠️ Не заменять на Game.GetUnassignedPlayerIDs().length: не доказано, что зритель
// туда не попадает. Если попадает, то на рейтинговой карте условие «есть
// нерассаженные» осталось бы истинным даже после успешного шафла, и отложенный
// автоассайн затащил бы зрителя в команду — уже ПОСЛЕ того, как команды посчитаны.
//--------------------------------------------------------------------------------------------------
function CountUnseatedPlayers()
{
	var ids = Game.GetUnassignedPlayerIDs();
	var count = 0;
	for ( var i = 0; i < ids.length; ++i )
	{
		if ( Players.IsValidPlayerID( ids[ i ] ) && !Players.IsSpectator( ids[ i ] ) )
		{
			++count;
		}
	}
...
function OnShufflePlayersPressed()
{
	...
	// ⚠️ Game.ShufflePlayerTeamAssignments() перемешивает ТОЛЬКО тех, кто уже в
	// команде, и никого не рассаживает сам. Обычно все сидят (автоассайн отработал
	// при создании панели), но не всегда: кто-то мог выйти из команды кнопкой
	// UnassignedPlayersButton или подключиться позже. Без этой строчки таких людей
	// шафл бы молча проигнорировал и оставил без команды.
	if ( CountUnseatedPlayers() > 0 )
	{
		Game.AutoAssignPlayersToTeams();
	}
	// Перемешивает уже рассаженных. Пати при этом НЕ сохраняются — в отличие от
	// автоассайна, это честный случайный шафл, что от кнопки и требуется.
...
(function()
{
	...
	//
	// ⚠️ Этот вызов ОСТАВЛЕН намеренно, и откладывать его не надо — проверялось.
	// Он рассаживает ТОЛЬКО тех, кто без команды (см. комментарий Valve у
	// OnAutoAssignPressed), поэтому перекрыть расстановку нашего шафла по MMR
	// (fate_mmr.lua) физически не может: после шафла все сидят, и повторный вызов
	// с чьей угодно панели — пустышка. Он же единственное, что рассаживает людей
	// на нерейтинговых картах (fate_ffa, fate_trio_rumble_3v3v3v3), где
	// IsRatedMap() = false и шафл не запускается вовсе.
	//
	// ⚠️ ЧЕГО ОТ НЕГО ЖДАТЬ: он намеренно держит пати в одной команде. Пока в
	// ComputeBalance стоял тай-брейк «меньше перестановок», балансировщик эту
	// группировку просто утверждал, и составы повторялись из матча в матч. Лечится
	// это в Lua (случайный выбор среди равноценных разбиений), а не здесь.
	if ( bAutoAssignTeams )
	{
		Game.AutoAssignPlayersToTeams();
	}
