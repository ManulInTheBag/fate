...
function karna_combo_vasavi_new:OnAbilityPhaseStart()
	local caster = self:GetCaster()
	StartAnimation(caster, {duration=4.3, activity=ACT_DOTA_CAST_GHOST_WALK, rate=1})
end

function karna_combo_vasavi_new:OnAbilityPhaseInterrupted()
	local caster = self:GetCaster()
    EndAnimation(caster)
end


function karna_combo_vasavi_new:GetCastRange(vLocation, hTarget)
	return self:GetSpecialValueFor("cast_range")
end

function karna_combo_vasavi_new:GetCastPoint()
	return self:GetSpecialValueFor("cast_point")
end

function karna_combo_vasavi_new:OnSpellStart()
	local caster = self:GetCaster()
	...
	local vasavi = caster:FindAbilityByName("karna_combo_vasavi_new")
	...
	Timers:CreateTimer(sound_delay-0.2, function()
...
function karna_combo_vasavi_new:OnProjectileThink(vLocation)
...
function karna_combo_vasavi_new:OnProjectileHit_ExtraData(hTarget, vLocation, table)
