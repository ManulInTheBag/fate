...
function modifier_jet_kappa:OnIntervalThink()
	...
	if true then
		...
		--self:VerticalMotion(self.parent, FrameTime())
		local enemy = FindUnitsInRadius(self.parent:GetTeam(), self.parent:GetAbsOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_CLOSEST, false)
		local damage = (self.parent.IsKikuIchimonjiAcquired and self.parent:GetAgility()*0.75 or 0) + self.parent:FindAbilityByName("okita_sandanzuki"):GetSpecialValueFor("base_damage") --self:GetAbility():GetSpecialValueFor("base_damage")+
		for i = 1,#enemy do
			if not enemy[i]:HasModifier("modifier_jet_checker") then
	    		...
	    		Timers:CreateTimer(0.8, function()
	       			DoDamage(self.parent, enemy[i], damage, DAMAGE_TYPE_PURE, 0, self.parent:FindAbilityByName("okita_jet"), false)
