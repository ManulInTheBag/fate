...
function modifier_laus_saint_ready_checker:OnCreated(args)
	if IsServer() then
	    ...
	    if caster:GetStrength() >= 29.1 and caster:GetAgility() >= 29.1 and caster:GetIntellect() >= 29.1 then
	    	if caster:FindAbilityByName("nero_aestus_domus_aurea"):IsCooldownReady() and caster:IsAlive() and caster:GetAbilityByIndex(3) ~= "nero_aestus_domus_aurea" and not caster:HasModifier("modifier_aestus_domus_aurea_nero") then	    		
	    		caster:SwapAbilities("nero_aestus_domus_aurea", "nero_heat", true, false)	    		
	    	end
	    end
	    if caster.UpgradeLSK then
	    	if caster:FindAbilityByName("nero_laus_saint_claudius_new"):IsCooldownReady() and caster:IsAlive() and caster:HasModifier("modifier_aestus_domus_aurea_nero") and caster:GetAbilityByIndex(3) ~= "nero_laus_saint_claudius_new" then	    		
	    		caster:SwapAbilities("nero_laus_saint_claudius_new", "nero_heat", true, false)   		
	    	end
	    end
	    if caster:HasModifier("modifier_ptb_attribute") and caster.UpgradeBase then
	    	if caster:IsAlive() and (caster:GetAbilityByIndex(0):GetName() == "nero_tres_new") then	    		
	    		caster:SwapAbilities("nero_tres_buffed", "nero_tres_new", true, false)	    		
	    	end
	    	if caster:IsAlive() and (caster:GetAbilityByIndex(1):GetName() == "nero_gladiusanus_new") then	    		
	    		caster:SwapAbilities("nero_gladiusanus_buffed", "nero_gladiusanus_new", true, false)	    		
	    	end
	    	if caster:IsAlive() and (caster:GetAbilityByIndex(2):GetName() == "nero_rosa_new") then	    		
	    		caster:SwapAbilities("nero_rosa_buffed", "nero_rosa_new", true, false)	    		
	    	end
	    	if caster:IsAlive() and (caster:GetAbilityByIndex(5):GetName() == "nero_spectaculi_initium") then	    		
	    		caster:SwapAbilities("nero_spectaculi_buffed", "nero_spectaculi_initium", true, false)	    		
	    	end
	    end
	end
end
...
function modifier_laus_saint_ready_checker:OnDestroy()
	if IsServer() then
		...
		if caster:GetAbilityByIndex(3):GetName() == "nero_aestus_domus_aurea" then
