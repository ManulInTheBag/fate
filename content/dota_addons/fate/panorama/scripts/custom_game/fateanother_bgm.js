...
g_GameConfig.duration = [481,187,327,219,142,183,143,233,212,247,241,280,224,288,187,318,270,344,255,375,292,184,202,142,162,215,140,126,249,133,185,238,237,184,212,174];
    ...
    g_GameConfig.BGMSchedule = $.Schedule(BGMduration, function(){
        ...
        if (g_GameConfig.bIsBGMOn === true){
            if (!g_GameConfig.bRepeat) {
                g_GameConfig.nextBGMIndex = Math.floor((Math.random() * 35) + 1);
            }
            PlayBGM();
        }
    });
...
{
    var index2 = Math.floor((Math.random() * 35) + 1);
